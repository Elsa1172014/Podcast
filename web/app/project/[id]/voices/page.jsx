      ...
      {mode === "school" && showCapture ? (
        ...
        <>
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            <p style={{ fontSize: 11, color: "#9AA6A3", margin: "0 0 6px" }}>ضروري لمقارنة حقيقية — إن اختلف النص، ستختلف مدة الكلام ونبرته تلقائيًّا بلا علاقة بجودة الاستنساخ.</p>
            <textarea value={testText} onChange={(e) => setTestText(e.target.value)} rows={2} placeholder="مثال: أهلًا بكم، اسمي أحمد وهذا صوتي..." style={{ width: "100%", padding: 10, borderRadius: 8, border: "1px solid #DCE4DF", fontFamily: "inherit", boxSizing: "border-box" }} />
            <button disabled={transcribing || (!file && !recordedBlob)} onClick={async () => {
              const blob = mode === "upload" ? file : recordedBlob;
              if (!blob) return setErr("سجّل أو ارفع صوتًا أولًا قبل الاستخراج التلقائي.");
              setTranscribing(true); setErr("");
              try {
                const audioBase64 = await blobToBase64(blob);
                const res = await fetch("/api/transcribe", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ audioBase64, mimeType: blob.type, accessToken: session.access_token }) });
                const data = await res.json();
                if (!res.ok) throw new Error(data.error);
                setTestText(data.text);
              } catch (e) {
                setErr("تعذّر الاستخراج التلقائي: " + e.message + " — اكتب النص يدويًّا بدلًا من ذلك.");
              }
              setTranscribing(false);
            }} style={{ marginTop: 6, padding: "6px 14px", borderRadius: 8, border: "1px solid #14746F", background: "#fff", color: "#14746F", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
              {transcribing ? "جارٍ الاستخراج…" : "🎙️ استخرج النص تلقائيًّا من التسجيل"}
            </button>
          </div>
            ...
            <div style={{ padding: 14, background: "#fff", borderRadius: 10, border: "1px solid #DCE4DF", marginBottom: 16 }}>
              ...
              {analysis && (
                <div style={{ marginTop: 14, padding: 16, background: "#fff", borderRadius: 12, border: "1px solid #DCE4DF" }}>
                  ...
                  <p style={{ fontSize: 11, color: "#9AA6A3", margin: 0 }}>{analysis.note}</p>
                </div>
              )}
            </div>
          )}
